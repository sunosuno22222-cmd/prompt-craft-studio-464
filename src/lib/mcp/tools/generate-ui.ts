...
export default defineTool({
  ...
  title: "Generate UI",
  description:
    "Generate a complete, self-contained React UI (App.jsx + styles.css) from a prompt using XZAFE AIcode (DeepSeek V3.1 Terminus via NVIDIA). Returns the raw model output containing <file path=\"...\">...</file> blocks.",
  inputSchema: {
  ...
  handler: async ({ prompt, framework, style }) => {
    ...
    if (!apiKey) {
      return {
        content: [{ type: "text", text: "NVIDIA_API_KEY is not configured on the server." }],
        isError: true,
      };
    }
